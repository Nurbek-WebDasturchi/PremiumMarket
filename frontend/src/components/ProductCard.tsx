    ...
    <motion.article whileHover={{ y: -6 }} className="glass group overflow-hidden rounded-xl p-3">
      <Link to={`/products/${product.slug}`} className="block">
        <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-900">
          ...
          {product.old_price && <span className="absolute left-3 top-3 rounded-full bg-rose-500 px-3 py-1 text-xs font-bold text-white">Sale</span>}
        </div>
      </Link>
      <div className="p-2">
        ...
        <div className="mt-3 flex items-end justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button title="Wishlist" onClick={() => toggle(product)} className="btn-soft h-10 w-10 p-0">
              <Heart className={saved ? 'h-4 w-4 fill-rose-500 text-rose-500' : 'h-4 w-4'} />
            </button>
            <button title="Add to cart" onClick={() => add(product)} className="btn-primary h-10 w-10 p-0">
