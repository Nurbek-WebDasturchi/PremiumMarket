...
export const Checkout = () => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Checkout failed. Log in and try again.');
    }
  };
    ...
    <motion.form {...pageTransition} onSubmit={submit} className="grid gap-6 lg:grid-cols-[1fr_22rem]">
      <div className="glass rounded-xl p-5">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          {Object.entries({ fullName: 'Full name', phone: 'Phone', city: 'City', address: 'Address' }).map(([key, label]) => (
        ...
        <p className="mt-4 rounded-lg bg-brand-500/10 p-4 text-sm text-brand-800 dark:text-brand-100">Demo checkout only. No real payment is charged.</p>
      </div>
      <aside className="glass h-fit rounded-xl p-5">
        ...
        <div className="flex justify-between border-t border-slate-200 pt-4 text-lg font-black dark:border-white/10"><span>Total</span><span>{money(total())}</span></div>
        <button className="btn-primary mt-5 w-full">Pay demo</button>
      </aside>
    </motion.form>
  );
};
